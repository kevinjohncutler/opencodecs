 ...
 * including leading zeros used in rdecomp_int, rdecomp_short and
 * rdecomp_byte
 */
    ...
    int nblock)       /* coding block size */
{
    ...
    */
    ...
    for (i = 0; i < nx; i += nblock) {
         ...
         * values. Note that this may overflow the integer variables -- that's
        ...
        for (j = 0; j < thisblock; j++) {
            ...
            diff[j] =
                (unsigned int)((pdiff < 0) ? ~(pdiff << 1) : (pdiff << 1));
            pixelsum += diff[j];
            lastpix = nextpix;
        }
        ...
        if (fs >= fsmax) {
             ...
             * Just write pixel difference values directly, no Rice coding at
             * all.
             */
            ...
            for (j = 0; j < thisblock; j++) {
                 ...
                 * code is output_nbits, moved into this routine to reduce
                 * overheads This code potentially breaks if FS>24, so I am
                 * limiting FS to 24 by choice of FSMAX above.
                 */
                if (fs > 0) {
                    lbitbuffer <<= fs;
                    lbitbuffer |= v & fsmask;
     ...
     */
    return (int)(buffer->current - buffer->start);
}
/*---------------------------------------------------------------------------*/
